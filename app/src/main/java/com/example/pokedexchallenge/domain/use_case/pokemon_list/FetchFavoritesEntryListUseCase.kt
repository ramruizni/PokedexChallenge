...
class FetchFavoritesEntryListUseCase @Inject constructor(
    private val dispatchers: DispatcherProvider,
    private val entryRepository: EntryRepository,
) {

    suspend operator fun invoke(): Flow<Resource<List<Entry>>> {
        return flow {
            emit(Resource.Loading(true))

            val result = entryRepository.fetchFavoritesEntryList()
